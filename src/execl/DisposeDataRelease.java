...
//			long ONE_HOUR = 3600;
			...
			while (anchorIterator.hasNext()) {
				...
				while (dateToTimeIterator.hasNext()) {
					...
					itemAnchorTimeCell.setCellValue(hourTime);
			...
			FileOutputStream output = new FileOutputStream("/Users/hanshihui/Documents/UYI_PROJECT/talentweb/SomeTest/src/execl/20180311_12麦时精到小时.xls");
			wb.write(output);
			wb.close();
			output.flush();
			output.close();
...
//		System.out.println("list.size " + whiteMap.size());
		...
		detailJSONToRoomJSON(whiteMap,"/Users/hanshihui/Documents/UYI_PROJECT/talentweb/SomeTest/src/execl/achorMicro0311_0312.txt");
		getFinalJSON();
		try {
			jsonToExcel();
		} catch (Exception e) {
